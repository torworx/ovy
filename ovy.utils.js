(function (context) {
     ...
     * ## Usage
     *
     *     var task = new ovy.util.DelayedTask(function(){
     *         alert(ovy.getDom('myInputField').value.length);
     *     });
     *
     *     // Wait 500ms before calling our function. If the user presses another key
     *     // during that 500ms, it will be cancelled and we'll wait another 500ms.
     *     ovy.get('myInputField').on('keypress', function(){
     *         task.{@link #delay}(500);
     *     });
     *
     * Note that we are using a DelayedTask here to illustrate a point. The configuration
     * option `buffer` for {@link ovy.util.Observable#addListener addListener/on} will
